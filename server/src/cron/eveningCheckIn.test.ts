...
import { createSession, completeSession, getPendingMessages, type DB } from '../db/index.js';
...
describe('runEveningCheckIn', () => {
  it('sends combined message when user missed today and has workout tomorrow', async () => {
    const userId = await seedTestUser(db);
    await seedTestPPL(db, userId);

    const today = new Date().toISOString().split('T')[0];
    const result = await runEveningCheckIn(db, today, 1, 3); // Monday missed, Wednesday next
  ...
  it('sends missed-only message when no workout tomorrow', async () => {
    const userId = await seedTestUser(db);
    await seedTestSchedule(db, userId); // Only Monday

    const today = new Date().toISOString().split('T')[0];
    const result = await runEveningCheckIn(db, today, 1, 4); // Monday missed, Thursday no workout
  ...
  it('sends pre-workout-only message when today was completed', async () => {
    const userId = await seedTestUser(db);
    await seedTestPPL(db, userId);
    ...
    const result = await runEveningCheckIn(db, today, 1, 3); // Monday completed, Wednesday next
  ...
  it('skips when no workout today or tomorrow', async () => {
    const userId = await seedTestUser(db);
    await seedTestSchedule(db, userId); // Only Monday

    const today = new Date().toISOString().split('T')[0];
    const result = await runEveningCheckIn(db, today, 4, 6); // Thursday, Saturday — no workouts

    expect(result.messaged).toBe(0);
    expect(result.skipped).toBe(1);
  });

  it('does not double-message for completed today + no workout tomorrow', async () => {
    const userId = await seedTestUser(db);
    await seedTestSchedule(db, userId); // Only Monday
    ...
    const today = new Date(session.started_at).toISOString().split('T')[0];
    const result = await runEveningCheckIn(db, today, 1, 4); // Monday completed, Thursday no workout
